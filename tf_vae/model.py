...
def conv_layer(input, shape, strides=[1, 1, 1, 1]):
    ...
    return tf.nn.relu(conv2d(input, W, strides) + b)
...
class ConvVAE(object):
    ...
    def __init__(self, z_size=128, batch_size=100, learning_rate=0.0001,
    ...
    def _build_graph(self):
        self.graph = tf.Graph()
        with self.graph.as_default():
            self.input_tensor = tf.placeholder(tf.float32, shape=[None, 120, 200, 1], name="input")
            # Encoder
            # h = tf.layers.conv2d(self.input_tensor, 32, 4, strides=2, padding="VALID", activation=tf.nn.relu, name="enc_conv1")
            # h = tf.layers.conv2d(h, 64, 4, strides=2, padding="SAME", activation=tf.nn.relu, name="enc_conv2")
            # h = tf.layers.conv2d(h, 128, 4, strides=2, padding="SAME", activation=tf.nn.relu, name="enc_conv3")
            # h = conv_to_fc(h)

            # encoder 2
            h = conv_layer(self.input_tensor, shape=[5, 5, 1, 32], strides=[1, 2, 2, 1])
            h = conv_layer(h, shape=[5, 5, 32, 64], strides=[1, 2, 2, 1])
            h = conv_layer(h, shape=[5, 5, 64, 128], strides=[1, 2, 2, 1])
            h = conv_to_fc(h)
            ...
            # Decoder
            h = tf.layers.dense(self.z, 15* 25 * 128, name="dec_fc")
            h = tf.reshape(h, [-1, 15, 25, 128])
            h = tf.layers.conv2d_transpose(h, 64, 4, strides=2, padding='SAME', activation=tf.nn.relu, name="dec_deconv1")
            h = tf.layers.conv2d_transpose(h, 32, 4, strides=2, padding='SAME',activation=tf.nn.relu, name="dec_deconv2")
            self.output_tensor = tf.layers.conv2d_transpose(h, 1, 4, strides=2, padding='SAME', activation=tf.nn.sigmoid, name="output")
            ...
            if self.is_training:
                ...
                # reconstruction loss
                # print(self.input_tensor.shape, self.output_tensor.shape)
